/* KallistiOS ##version##
   ...
   copyright umbrella (along with KOS-specific changes) for release under
   the BSD-like license.

   This beast looks daunting, but a lot of it is comments and it has some
   very nice thread safety support.

   Unlike previous versions, most of what was malloc.h is now left in place
   here, and minus culling out a bunch of stuff we don't use, I tried to
...
/* Each memory pool block will look something like this:
   ...
   char[BUFFER_SIZE]   post-buffer no-touch zone
 */
...
int mem_check_all(void) {
    ...
    LIST_FOREACH(ctl, &block_list, list) {
        rvp = mem_check_block((void *)(((uint32)ctl) + BUFFER_SIZE));
...
/* Enable this define if you want costly malloc debugging (buffer
   sentinel checking, block leak checking, etc) */
/* #define KM_DBG */
